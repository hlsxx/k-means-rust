...
fn k_means(points: &Vec<Point>, cluster_count: usize, iterations: u8) -> (Arc<Vec<RwLock<Point>>>, Vec<Arc<RwLock<Vec<Point>>>>) {
  let mut rng = thread_rng();

  // Init some random Points of centroids
  let centroids: Arc<Vec<RwLock<Point>>> = Arc::new(
    (0..cluster_count)
        .map(|_| RwLock::new(Point {
            x: rng.gen_range(0.0..MATRIX.x),
            y: rng.gen_range(0.0..MATRIX.y),
        }))
        .collect::<Vec<RwLock<Point>>>()
);  
  ...
  for _ in 0..iterations {    
    ...
    points.par_iter().for_each(|point| {
      ...
      for (i, centroid) in centroids.iter().enumerate() {
        let distance = point.distance(&centroid.read().unwrap());
    ...
    cluster_locks.par_iter().enumerate().for_each(|(i, cluster_lock)| {
      ...
      let centroid = cluster.par_iter()
        ...
        *centroids[i].write().unwrap() = centroid;
    });
